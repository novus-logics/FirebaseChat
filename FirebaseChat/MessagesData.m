...
- (void)createDictionaryFromVideoMessage:(NSString *)key completionBlock:(void (^)(NSDictionary *dict))completionBlock {
    ...
    JSQVideoMediaItem *videoItem = (JSQVideoMediaItem *)self.media;
    [[FIRStorageManager sharedManager] saveVideoMessage:videoItem.fileURL withName:key successBlock:^(NSString *url, NSError *error) {
        if (url && !error) {
            
            UIImage *thumbnail = [self thumbnailImageFromFileUrl:videoItem.fileURL];
            if (thumbnail) {
                [[CloudinaryManager sharedManager] uploadImage:thumbnail withName:key completionBlock:^(NSString *url1, NSError *error) {
                    if (url && !error) {
                        NSDictionary *post = @{@"uid": myID,
                                               @"author": username,
                                               @"date":dateString,
...
- (UIImage *)thumbnailImageFromFileUrl:(NSURL *)fileUrl
{
    AVAsset *asset = [AVAsset assetWithURL:fileUrl];
    AVAssetImageGenerator *imageGenerator = [AVAssetImageGenerator assetImageGeneratorWithAsset:asset];
    CGImageRef imageRef = [imageGenerator copyCGImageAtTime:CMTimeMake(1, 60) actualTime:nil error:nil];
...
+ (JSQMessage*) createFromDict: (NSDictionary*)messageDict {
    ...
    NSString *videoUrl = [messageDict objectForKey:@"videoUrl"];
    ...
    if (!typeOf) {
        ...
        if (videoUrl) {
            AsyncVideoMediaItem *videoItem = [[AsyncVideoMediaItem alloc] initWithURL:text andVideo:videoUrl];
            return [[JSQMessage alloc] initWithSenderId:senderID senderDisplayName:displayName date:date media:videoItem];
        }else {
...
- (void)sendMessage: (JSQMessage*)message {
    ...
    if (message.isMediaMessage) {
        if ([message.media isKindOfClass:[JSQVideoMediaItem class]]) {
            ...
            [message createDictionaryFromVideoMessage:key completionBlock:^(NSDictionary *dict) {
                if (dict) {
