...
@property (strong, nonatomic) FIRStorageReference *videosPathRef;
@end
...
- (void)initStorageReference {
    ...
    self.videosPathRef = [storageRef child:ALL_VIDEOS];
}
...
- (void)saveVideoMessage: (NSURL*)urlVideo
    ...
    FIRStorageReference *videosRef = [self.videosPathRef child:[name stringByAppendingString:@".mov"]];
    
    [videosRef putFile:urlVideo metadata:nil completion:^(FIRStorageMetadata * _Nullable metadata, NSError * _Nullable error) {
        if (error != nil)
            successBlock(nil, error);
        else
            successBlock(metadata.downloadURL.absoluteString, nil);
    }];
