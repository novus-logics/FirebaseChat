...
- (void)collectionView:(JSQMessagesCollectionView *)collectionView didTapMessageBubbleAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(message.isMediaMessage) {
        ...
        if ([message.media isKindOfClass:[AsyncPhotoMediaItem class]]) {
            ...
            image = media.image;
        }else if ([message.media isKindOfClass:[AsyncVideoMediaItem class]]) {
            AsyncVideoMediaItem *media = (AsyncVideoMediaItem *)message.media;
...
didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info {
    ...
    if ([mediaType isEqualToString:(NSString *)kUTTypeMovie]) {
        NSURL *urlVideo =  [info valueForKey:UIImagePickerControllerMediaURL];
        JSQVideoMediaItem *video = [[JSQVideoMediaItem alloc] initWithFileURL:urlVideo isReadyToPlay:false];
        JSQMessage *mediaMessage = [JSQMessage messageWithSenderId:self.senderId displayName:self.senderDisplayName media:video];
